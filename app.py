...
# ==================== SESSION MANAGEMENT ====================
def init_session():
    # التأكد من تحميل البيانات الافتراضية لتجنب الأخطاء
...
# تحميل صامت للموديل (بدون Spinner مزعج كل مرة)
model, base_model, metrics, feature_cols = get_model()

# ==================== HEADER & LOGO (FIXED) ====================
# الحل الجذري للوجو: استخدام الأعمدة لتوسيط الصورة
c1, c2, c3 = st.columns([1, 2, 1]) # تقسيم الصفحة لثلاثة أعمدة
with c2: # العمود الأوسط
...
st.caption("🛠️ " + ("اختبار سريع (اضغط لتغيير الحالة فوراً):" if lang == "ar" else "Quick Test (Click to update status):"))
b1, b2, b3 = st.columns(3)

# عند الضغط، يتم تحديث البيانات وعمل Rerun، مما يؤدي لتحديث التحليل تلقائياً بالأسفل
...
# ==================== AUTO ANALYSIS LOGIC (ALWAYS RUNNING) ====================
# تم حذف الزر، التحليل يتم تلقائياً الآن
...
# 2. Predict & Display Results
try:
    pred_class = int(model.predict(input_df)[0])
    pred_proba = model.predict_proba(input_df)[0]
    action = CLINICAL_ACTIONS[pred_class]
    
    # 3. RESULT CARD (With FLASHING if HIGH RISK)
    # استخدام كلاس critical-alert-card في حالة الخطر للوميض
